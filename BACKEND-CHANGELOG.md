...
- **Let-out schedule:** `dim_schedule` is computed from the whole member set so the `prime` pulse spans the **9pm dinner let-out through the district's latest close**, with a `prime_weekend` that runs later Fri/Sat (clubs to ~4am). Hour ranges wrap past midnight. `district_runtime_meta()` recomputes `dim_schedule`/`best_hours`/`rationale` at read time, so a schedule edit takes effect on the next GET with no rebuild.
