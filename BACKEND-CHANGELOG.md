...
The codebase already had the right denoiser — `_same_weekday_blended_scores` (today `0.40` + the same weekday/time from the prior 3 weeks at `0.20` each, re-normalized over present samples) — but it ran **only** in the exact-store/attestation path, **never** in what drivers see (`/frame/{idx}` → `build_single_frame_for_month`, which built one timestamp from parquet with no blend). It was also scoped to citywide only.

This pass:
- **Wires the blend into the driver-served path.** `build_single_frame_for_month` now scans the requested bin **plus up to 3 prior same-weekday bins**, each with its own narrow `±60 min` pickup window (cheap — a few short scans, not a multi-week span; the single SQL pass already yields every mode's columns), and blends them via the shared helper. Each frame is now a ~4-week same-weekday average instead of one noisy day.
- **Adds per-zone volatility damping.** From the same today+prior `pickups_now` samples it computes the coefficient of variation and shaves up to **20%** off the zone's score/rating when demand is inconsistent (busy some weeks, dead others), scaling from CoV `0.50` (no damp) to `1.50` (full 20%). "Busy" now means *reliably* busy; a one-week spike with dead priors is pulled down, a reliably-busy zone is untouched. The signal is **mode-independent**, so one damp factor per zone is applied to **every** color.
- **System-wide — all zones, all colors.** The blend/damp column sets are **derived from `V3_PROFILE_CONFIG`** (so they can't drift) and cover all 7 user-visible modes: citywide_v3 (+ trap-candidate variant), the 5 borough v3 modes, and trips_45plus_v3. The 6 recalibrated modes are damped at their **score** field (recalibration re-ranks from it); trips_45plus_v3 is served straight from the SQL, so it's damped at its **rating** field. Confidence fields are blended (denoised) but never damped.
- **Refactor:** extracted the pure blend/damp math into `_blend_same_weekday_rows` + `_same_weekday_consistency_damp` + `_apply_blended_columns_to_row_map`, shared by **both** the parquet serve path and the exact-store path so both apply the identical blend+damp. (The two paths still compute per-bin scores over different windows — narrow ±60 min on the serve side, full-month on the store side — but that divergence is **pre-existing** and already absorbed by the integer-rating comparison in `_normalize_frame_payload_for_compare`, which covers every `_v3` rating/bucket/color; averaging four same-weekday bins does not widen it.) No attestation change needed.
- **Fixed a latent store-path no-op:** `_same_weekday_blended_scores` keyed "today" off `str(exact_bin_local_ts)`, but that column is a TIMESTAMPTZ whose `str()` is `"… 18:00:00+00:00"` — never equal to the T-formatted `requested_frame_time` — so the store-path blend silently returned `{}` and never applied (which is *why* attestation never tripped before: neither path blended). Now it derives the key via `strftime(exact_bin_local_ts AT TIME ZONE 'UTC', '%Y-%m-%dT%H:%M:%S')` (timezone-independent), so the store path genuinely blends, matching the serve path.
- **Cache invalidation:** the fix lives in `build_hotspot.py`, which `_rating_logic_version_token()` did **not** hash (it only hashed `zone_earnings_engine` + `zone_mode_profiles`) — so without action, deploys would keep serving stale frames. Added `build_hotspot` to the hashed modules; the existing startup "version changed → regenerate" path now fires on deploy, purging + rebuilding frame caches with the blend.
- **Cold-start (documented):** a frame whose 3 prior weeks fall in the previous month degrades to today-only — exactly matching the current-month-only exact store, so the attestation comparison stays consistent. Widening both paths to the prior month is a deliberate future enhancement (the two paths must move together to avoid spurious store retirement).
- All knobs (blend weights, `MAX_DAMP`, `CV_LO`/`CV_HI`) are named constants for easy tuning.
- Tests: new `tests/test_same_weekday_blend_damping.py` (9 tests) — pure blend/damp math (multi-mode uniform damping, confidence exempt, spiky-pulled-below-today, cold-start, None-safe/finite, damp-curve edges, rating rounding); a **contract test running the real engine SQL** to prove every blend/damp column exists in the output (it caught that trips_45plus_v3 has no confidence column); and an end-to-end test through `build_single_frame_for_month` asserting a spiky zone is rated **below** a steady one. Full suite green (79 passed, 15 pre-existing xfail).
