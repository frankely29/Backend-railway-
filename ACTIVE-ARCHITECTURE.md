...
- `build_hotspot.py` now runs two backend-only scoring paths during frame generation:
  ...
  - shared HVFHV factual shadow engine (`zone_earnings_engine.py`) with emitted citywide + Manhattan v2 shadow fields for future phases.
...
- Backend shadow output now includes `manhattan_v2` alongside `citywide_v2`; Manhattan-visible cutover is handled in frontend/runtime phases while citywide remains the default outside Manhattan scope.
