...
from build_hotspot import _validate_popup_metric_consistency, build_hotspots_frames
...
def _write_null_pay_sample_parquet(path: Path) -> None:
    con = duckdb.connect(database=":memory:")
    con.execute(
        """
        CREATE TABLE trips AS
        SELECT * FROM (
            VALUES
                (46, 132, TIMESTAMP '2025-01-06 08:05:00', TIMESTAMP '2025-01-06 08:00:00', NULL, 900.0, 4.0, 0, 0),
                (46, 132, TIMESTAMP '2025-01-06 08:15:00', TIMESTAMP '2025-01-06 08:10:00', NULL, 840.0, 3.5, 0, 0)
        ) AS t(PULocationID, DOLocationID, pickup_datetime, request_datetime, driver_pay, trip_time, trip_miles, shared_match_flag, shared_request_flag)
        """
    )
    con.execute(f"COPY trips TO '{path.as_posix()}' (FORMAT PARQUET)")
...
def test_shadow_sql_keeps_zone_bin_when_driver_pay_is_all_null(tmp_path: Path) -> None:
    parquet_path = tmp_path / "sample_null_pay.parquet"
    _write_null_pay_sample_parquet(parquet_path)
    ...
    assert rows, "Expected shadow rows to exist even when driver_pay is null"
    assert any(float(row["pickups_now"]) >= 1.0 for row in rows)
    for row in rows:
        assert row["median_driver_pay"] is None
        for field_name in (
            "zone_area_sq_miles",
            "pickups_per_sq_mile_now",
            "pickups_per_sq_mile_next",
...
def test_popup_metric_validation_raises_with_structured_diagnostics(caplog: pytest.LogCaptureFixture) -> None:
    features = [{
        "type": "Feature",
        "properties": {
            "LocationID": 46,
            ...
            "pickups_now_shadow": None,
            "next_pickups_shadow": 2.0,
            "zone_area_sq_miles_shadow": 0.25,
            "pickups_per_sq_mile_now_shadow": 8.0,
            "pickups_per_sq_mile_next_shadow": 8.0,
        },
    }]
    diagnostics_by_location_id = {
        46: {
            "shadow_sql_row_exists": False,
            "geometry_area_row_exists": True,
            "pickups_now_shadow": None,
            "next_pickups_shadow": None,
            "zone_area_sq_miles_shadow": None,
            "pickups_per_sq_mile_now_shadow": None,
            "pickups_per_sq_mile_next_shadow": None,
        }
    }

    with pytest.raises(RuntimeError, match="Popup metric consistency validation failed"):
        _validate_popup_metric_consistency(
            features,
            "2025-01-06T08:00:00",
            diagnostics_by_location_id=diagnostics_by_location_id,
        )

    log_text = "\n".join(record.getMessage() for record in caplog.records)
    assert '"LocationID": 46' in log_text
    assert '"zone_name": "City Island"' in log_text
    assert '"borough": "Bronx"' in log_text
    assert '"frame_time": "2025-01-06T08:00:00"' in log_text
    assert '"shadow_sql_row_exists": false' in log_text
    assert '"geometry_area_row_exists": true' in log_text
    assert '"pickups_now_shadow": null' in log_text
